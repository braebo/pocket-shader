// src/pocket-shader.ts
var PocketShader = class {
  /**
  ...
  _builtinUniformLocations = /* @__PURE__ */ new Map();
...
precision mediump float;uniform float u_time;in vec2 vUv;out vec4 color;void main() {color = vec4(vUv, 0.5 + 0.5 * sin(u_time), 1.0);}`;
  ...
  _validateUniforms(source) {
    ...
    while ((match = uniformRegex.exec(source)) !== null) {
      const [, , name] = match;
      if ([...this._builtinUniformLocations.keys(), ...Object.keys(this.uniforms)].some(
        (k) => k === name
      )) {
        continue;
      }
...
function throttledDebounce(fn, throttleMs = 50, debounceMs = 75) {
  let debounceTimeout;
  let lastExecution = 0;
