...
vec3 sceneNorm(vec3 p) {
    ...
    float d = sceneDist(p);
    return normalize(vec3(sceneDist(p + e.xyy) - sceneDist(p - e.xyy), sceneDist(p + e.yxy) - sceneDist(p - e.yxy), sceneDist(p + e.yyx) - sceneDist(p - e.yyx)));
}
...
void main() {
    ...
    if (t < 10.0) {
        ...
        // Previous code:
        // fragColor.rgb = mix(vec3(0.15, 0, 0), vec3(1), ao) * shad * diff * 1.1;
        fragColor = vec4(mix(vec3(0.15, 0.0, 0.0), vec3(1.0), ao) * 1.1, 1.0); // Annotated new color setting
    } else {
        // Background color if no intersection
        // Previous code:
        // fragColor.rgb += emit;
        fragColor = vec4(0.0, 0.0, 0.0, 1.0); // Annotated new background color setting
    }
